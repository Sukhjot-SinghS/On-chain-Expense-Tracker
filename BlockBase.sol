...
contract ExpenseTracker {
    ...
    address[] public  registeredPeople;
    ...
    /**
 * @dev Update a registered person's name
 * @param _newName The new name to set for the person
 */
function updatePersonName(string memory _newName) public {
    // Validate that the new name isn't empty
    require(bytes(_newName).length > 0, "Name cannot be empty");

    // Check if this address has been registered
    require(
    ...
    // Emit an event for front-end apps
    ...
    function returnnameofperson(address _addr )public view returns(string memory){
        require(
            people[msg.sender].walletAddress != address(0),
            "Person is not registered"
        );
